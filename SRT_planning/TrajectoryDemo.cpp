    ...
    Curve1d lon, Curve1d lat, std::vector<PathPoint>& ref)
{
    ...
    ref_points = ref;
    // 在这里写cost的计算!
    double lon_s_cost = 100 / lon.forward();            // Cost for lon forward distance
    double avg_speed_cost = 300 - 10 * lon.avg_speed(); // Cost for lon average speed
    double offset_cost = 0.0;                           // Cost for lat offset 
